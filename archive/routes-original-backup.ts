...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/capsules/available-with-status", async (_req, res) => {
    try {
      ...
      const capsulesWithStatus = displayableCapsules.map(capsule => ({
        ...capsule,
        canAssign: capsule.cleaningStatus === "cleaned" && capsule.isAvailable
      }));
  ...
  // Get capsules that need attention (cleaning, maintenance, etc.)
  app.get("/api/capsules/needs-attention", async (_req, res) => {
    try {
      const checkedInGuests = await storage.getCheckedInGuests();
      const occupiedCapsules = new Set(checkedInGuests.data.map(guest => guest.capsuleNumber));
      
      // Get all capsules and find ones that need attention
      const allCapsules = await storage.getAllCapsules();
      const capsulesNeedingAttention = allCapsules.filter(capsule => {
        // Skip if occupied by a guest
        if (occupiedCapsules.has(capsule.number)) {
          return false;
        }
        
        // Include if needs cleaning
        if (capsule.cleaningStatus === "to_be_cleaned") {
          return true;
        }
        
        // Include if not available (might have problems or issues)
      ...
      res.json(capsulesNeedingAttention);
    } catch (error) {
      console.error("Error getting capsules needing attention:", error);
      res.status(500).json({ message: "Failed to get capsules needing attention" });
    }
  });
  ...
  // Undo capsule cleaning (mark as needs cleaning)
  app.post("/api/capsules/:number/undo-cleaned", securityValidationMiddleware, async (req, res) => {
    try {
      const { number: capsuleNumber } = req.params;
      
      const capsule = await storage.markCapsuleNeedsCleaning(capsuleNumber);
      ...
      res.json({
        message: "Capsule cleaning undone successfully",
        capsule
      });
    } catch (error) {
      console.error('Error undoing capsule cleaning:', error);
      res.status(500).json({ message: "Failed to undo capsule cleaning" });
    }
  });
  ...
  app.post("/api/guests/checkin", 
    ...
    try {
      ...
      if (!availableCapsuleNumbers.includes(validatedData.capsuleNumber)) {
        // Check if it's because the capsule hasn't been cleaned yet
        const uncleanedCapsules = await storage.getUncleanedAvailableCapsules();
        const uncleanedCapsuleNumbers = uncleanedCapsules.map(c => c.number);
        
        if (uncleanedCapsuleNumbers.includes(validatedData.capsuleNumber)) {
          // Create admin notification about uncleaned capsule blocking assignment
          await storage.createAdminNotification({
            type: "cleaning_required",
            title: "Capsule Assignment Blocked - Cleaning Required",
            message: `Admin attempted to assign ${validatedData.name} to capsule ${validatedData.capsuleNumber}, but the capsule has not been cleaned yet. Please clean the capsule before it can be assigned to guests.`,
            capsuleNumber: validatedData.capsuleNumber,
            isRead: false
          });
          
          return res.status(400).json({ 
            message: `Capsule ${validatedData.capsuleNumber} has not been cleaned yet and cannot be assigned to guests. Please clean the capsule first.`,
