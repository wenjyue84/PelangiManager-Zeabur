  ...
  if (isSuccess) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-orange-50 to-pink-50 py-8">
        <div className={`mx-auto px-4 ${isMobile ? 'max-w-md' : 'max-w-2xl'}`}>
          <Card className="shadow-xl">
            <CardContent className={`${isMobile ? 'p-6' : 'p-8'}`}>
              <div className="text-center mb-6">
                <div className={`${isMobile ? 'text-3xl mb-3' : 'text-4xl mb-4'}`}>🎉</div>
                <h1 className={`${isMobile ? 'text-2xl' : 'text-3xl'} font-bold text-gray-900 mb-2`}>{t.goodDay}</h1>
                <div className={`${isMobile ? 'text-xl mb-3' : 'text-2xl mb-4'}`}>🎉</div>
              </div>

              <div className={`bg-gradient-to-r from-orange-100 to-pink-100 rounded-xl mb-6 text-center ${
                isMobile ? 'p-4' : 'p-6'
              }`}>
                <h2 className={`${isMobile ? 'text-lg' : 'text-xl'} font-bold text-gray-800 mb-2 flex items-center justify-center gap-2`}>
                  {t.welcomeHostel} <span className={isMobile ? 'text-xl' : 'text-2xl'}>🌈</span>
                </h2>
                <div className="space-y-2 text-gray-700">
                  <div className={`flex items-center justify-center gap-2 ${isMobile ? 'flex-col text-sm' : ''}`}>
                    <MapPin className="h-4 w-4" />
                    <span className="font-medium">{t.address}</span>
                    <span className={isMobile ? 'text-center' : ''}>26A, Jalan Perang, Taman Pelangi, 80400 Johor Bahru</span>
                  </div>
                </div>
              </div>
              ...
              <div className={`grid gap-4 mb-6 ${
                ...
                {settings?.guideHostelPhotosUrl && (
                  <Button variant="outline" className={`flex items-center gap-2 h-auto ${
                    isMobile ? 'py-4 px-6 text-base' : 'py-3 px-4'
                  }`} onClick={() => window.open(settings.guideHostelPhotosUrl, '_blank')}>
                    <Camera className="h-4 w-4" />
                    <span className={isMobile ? 'text-base' : 'text-sm'}>{t.hostelPhotos}</span>
                  </Button>
                )}
                {settings?.guideGoogleMapsUrl && (
                  <Button variant="outline" className={`flex items-center gap-2 h-auto ${
                    isMobile ? 'py-4 px-6 text-base' : 'py-3 px-4'
                  }`} onClick={() => window.open(settings.guideGoogleMapsUrl, '_blank')}>
                    <Globe className="h-4 w-4" />
                    <span className={isMobile ? 'text-base' : 'text-sm'}>{t.googleMaps}</span>
                  </Button>
                )}
                {settings?.guideCheckinVideoUrl && (
                  <Button variant="outline" className={`flex items-center gap-2 h-auto ${
                    isMobile ? 'py-4 px-6 text-base' : 'py-3 px-4'
                  }`} onClick={() => window.open(settings.guideCheckinVideoUrl, '_blank')}>
                    <Video className="h-4 w-4" />
                    <span className={isMobile ? 'text-base' : 'text-sm'}>{t.checkInVideo}</span>
                  </Button>
                )}
          ...
          <CardContent>
            ...
            {isMobile && (
              <div className="mb-4 p-4 bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 rounded-lg">
                ...
                <p className="text-xs text-blue-700 mb-2">
                  Welcome! You're using the mobile version. For the best experience:
                </p>
                <ul className="text-xs text-blue-700 space-y-1">
                  <li>• 📱 Hold your phone in portrait mode</li>
                  <li>• 📸 Take a clear photo of your document</li>
                  <li>• ✋ Use your thumb for better control</li>
                </ul>
              </div>
            )}
            ...
            <div className="mb-4 p-4 bg-blue-50 border border-blue-200 rounded-lg">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm">
                <div className={`flex items-center gap-2 ${(watchedIcNumber || watchedPassportNumber) ? 'text-green-700' : 'text-gray-500'}`}>
                  <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center ${
                    (watchedIcNumber || watchedPassportNumber) ? 'border-green-500 bg-green-500' : 'border-gray-300'
                  }`}>
                    {(watchedIcNumber || watchedPassportNumber) && <CheckCircle className="w-3 h-3 text-white" />}
                  </div>
                  <span>Select document type</span>
                </div>
                <div className={`flex items-center gap-2 ${(icDocumentUrl || passportDocumentUrl) ? 'text-green-700' : 'text-gray-500'}`}>
                  <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center ${
                    (icDocumentUrl || passportDocumentUrl) ? 'border-green-500 bg-green-500' : 'border-gray-300'
                  }`}>
                    {(icDocumentUrl || passportDocumentUrl) && <CheckCircle className="w-3 h-3 text-white" />}
                  </div>
                  <span>Upload document photo</span>
                </div>
                <div className={`flex items-center gap-2 ${form.watch("nameAsInDocument") ? 'text-green-700' : 'text-gray-500'}`}>
                  <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center ${
                    form.watch("nameAsInDocument") ? 'border-green-500 bg-green-500' : 'border-gray-300'
                  }`}>
                    {form.watch("nameAsInDocument") && <CheckCircle className="w-3 h-3 text-white" />}
                  </div>
                  <span>Fill personal information</span>
                </div>
                <div className={`flex items-center gap-2 ${form.watch("paymentMethod") ? 'text-green-700' : 'text-gray-500'}`}>
                  <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center ${
                    form.watch("paymentMethod") ? 'border-green-500 bg-green-500' : 'border-gray-300'
                  }`}>
                    {form.watch("paymentMethod") && <CheckCircle className="w-3 h-3 text-white" />}
                  </div>
              ...
              <div className="flex items-start gap-2 text-amber-800 text-sm">
                ...
                <div>
                  ...
                  <ul className="list-disc list-inside space-y-1">
                    <li><strong>📸 Document Photo Required:</strong> You must upload a photo of either your IC or passport</li>
            ...
            <form onSubmit={(e) => {
              ...
              // Validate document selection and upload before submission
              if (!(watchedIcNumber || watchedPassportNumber)) {
                toast({
                  title: "Document Type Required",
                  description: "Please select either IC or passport document type to continue.",
                  variant: "destructive",
                });
                return;
              }
              ...
              if (!icDocumentUrl && !passportDocumentUrl) {
                toast({
                  ...
                  description: "Please upload a photo of your document to continue.",
                  variant: "destructive",
                });
                return;
              }
              ...
              if (formData.icNumber === "000000000000") {
                form.setValue("icNumber", "");
              }
              if (formData.passportNumber === "PASSPORT123") {
                form.setValue("passportNumber", "");
              }
                ...
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500 mt-1 mb-3">
                      You must upload a photo of either your IC or passport. Choose one document type below and upload a clear photo.
                    </p>
                    ...
                    <div className="mb-4 p-3 bg-amber-50 border border-amber-200 rounded-lg">
                      <div className="flex items-start gap-2 text-amber-800 text-sm">
                        ...
                        <div>
                          <div className="font-medium mb-1">📋 Important: Document Upload Required</div>
                          <p>All guests must provide a photo of either their IC or passport for check-in. This is mandatory for security and verification purposes.</p>
                        </div>
                      </div>
                    </div>
                     ...
                     <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
                       <div className={`p-4 border-2 rounded-lg cursor-pointer transition-all duration-200 ${
                         watchedIcNumber ? 'border-blue-500 bg-blue-50 shadow-md' : 'border-gray-200 hover:border-gray-300 hover:shadow-sm'
                       } ${isMobile ? 'min-h-[80px]' : ''}`}>
                         <div className="flex items-center gap-3">
                           <input
                             ...
                             id="docTypeIc"
                             name="documentType"
                             checked={!!watchedIcNumber}
                             onChange={() => {
                               // Clear any existing document URLs when switching
                               if (passportDocumentUrl) {
                                 setPassportDocumentUrl("");
                                 form.setValue("passportDocumentUrl", "");
                               }
                               ...
                               if (!watchedIcNumber) {
                                 form.setValue("icNumber", "000000000000"); // Placeholder IC number
                                 form.setValue("passportNumber", "");
                               }
                             }}
                             className={`text-blue-600 ${isMobile ? 'w-6 h-6' : 'w-5 h-5'}`}
                           />
                           <label htmlFor="docTypeIc" className="text-sm font-medium cursor-pointer flex-1">
                             <div className="flex flex-col">
                               <span className="font-semibold">🆔 IC Document</span>
                               <span className="text-xs text-gray-500 mt-1">Malaysian IC</span>
                               <span className={`text-xs text-blue-600 mt-1 ${isMobile ? 'font-medium' : ''}`}>
                                 {isMobile ? '👆 Tap to select' : 'Tap to select'}
                               </span>
                             </div>
                           </label>
                         </div>
                       ...
                       <div className={`p-4 border-2 rounded-lg cursor-pointer transition-all duration-200 ${
                         watchedPassportNumber ? 'border-blue-500 bg-blue-50 shadow-md' : 'border-gray-200 hover:border-gray-300 hover:shadow-sm'
                       } ${isMobile ? 'min-h-[80px]' : ''}`}>
                         <div className="flex items-center gap-3">
                           <input
                             type="radio"
                             ...
                             onChange={() => {
                               ...
                               if (!watchedPassportNumber) {
                                 form.setValue("passportNumber", "PASSPORT123"); // Placeholder passport number
                                 form.setValue("icNumber", "");
                               }
                             }}
                             className={`text-blue-600 ${isMobile ? 'w-6 h-6' : 'w-5 h-5'}`}
                           />
                           <label htmlFor="docTypePassport" className="text-sm font-medium cursor-pointer flex-1">
                             <div className="flex flex-col">
                               <span className="font-semibold">🛂 Passport Document</span>
                               <span className="text-xs text-gray-500 mt-1">International Passport</span>
                               <span className={`text-xs text-blue-600 mt-1 ${isMobile ? 'font-medium' : ''}`}>
                                 {isMobile ? '👆 Tap to select' : 'Tap to select'}
                               </span>
                             </div>
                    ...
                    {(icDocumentUrl || passportDocumentUrl) ? (
                      <div className="mt-2 p-3 bg-green-100 border border-green-300 rounded-lg">
                        ...
                        <ObjectUploader
                          ...
                          buttonClassName="mt-2 w-full h-10 text-sm bg-gray-600 hover:bg-gray-700"
                        >
                      ...
                      <div className={`mt-2 border-2 border-dashed rounded-lg p-4 text-center ${
                        (watchedIcNumber || watchedPassportNumber) ? 'border-gray-300' : 'border-orange-300 bg-orange-50'
                      }`}>
                        <Camera className={`h-8 w-8 mx-auto mb-2 ${
                          (watchedIcNumber || watchedPassportNumber) ? 'text-gray-400' : 'text-orange-400'
                        }`} />
                        <p className={`text-sm mb-2 ${
                          (watchedIcNumber || watchedPassportNumber) ? 'text-gray-500' : 'text-orange-600 font-medium'
                        }`}>
                          {watchedIcNumber ? 'Upload a clear photo of your IC' : 
                           watchedPassportNumber ? 'Upload a clear photo of your passport' : 
                           '⚠️ Document upload is required for check-in'}
                        </p>
                        ...
                        {(watchedIcNumber || watchedPassportNumber) ? (
                          <div className="space-y-3">
                            <ObjectUploader
                              ...
                              buttonClassName="w-full h-12 text-base bg-blue-600 hover:bg-blue-700"
                            >
                            ...
                            <div className={`text-center space-y-1 ${
                              isMobile 
                                ? 'p-3 bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 rounded-lg' 
                                : 'text-xs text-gray-500'
                            }`}>
                              <p className={`${isMobile ? 'text-blue-800 font-medium' : ''}`}>
                                💡 <strong>Mobile Tips:</strong>
                              </p>
                              <p className={isMobile ? 'text-blue-700 text-sm' : ''}>• Take photo in good lighting</p>
                              <p className={isMobile ? 'text-blue-700 text-sm' : ''}>• Avoid glare and shadows</p>
                              <p className={isMobile ? 'text-blue-700 text-sm' : ''}>• Ensure all text is clearly visible</p>
                              {isMobile && (
                                <p className="text-blue-800 font-medium text-sm">• ✨ Tap the upload button above to start</p>
                              )}
                            </div>
                          </div>
                        ) : (
                          <div className="text-sm text-orange-600 font-medium">
                            Please select a document type above to proceed
                          </div>
                        )}
                      </div>
                    )}
              ...
              <div className="sticky bottom-0 left-0 right-0 z-10 -mx-6 px-6 py-3 bg-gradient-to-t from-background via-background/95 to-transparent">
                  ...
                  <div className={`mb-3 p-3 border rounded-lg ${
                    ...
                    <div className="flex items-center gap-2 text-red-700 text-sm">
                      ...
                      <span className={`font-medium ${isMobile ? 'text-base' : ''}`}>
                        {isMobile ? '📱 ' : '⚠️ '}Please select a document type and upload your document to continue
                      </span>
                    </div>
                  </div>
                )}
                
                {!icDocumentUrl && !passportDocumentUrl && (watchedIcNumber || watchedPassportNumber) && (
                  <div className={`mb-3 p-3 border rounded-lg ${
                    isMobile 
                      ? 'bg-gradient-to-r from-red-50 to-pink-50 border-red-300 shadow-sm' 
                      : 'bg-red-50 border-red-200'
                    ...
                    <div className="flex items-center gap-2 text-red-700 text-sm">
                      <Info className="h-4 w-4" />
                      <span className={`font-medium ${isMobile ? 'text-base' : ''}`}>
                        {isMobile ? '📱 ' : '⚠️ '}Please upload your document photo to continue
                      </span>
                    </div>
                  </div>
                )}
                ...
                <Button 
                  ...
                  className={`w-full text-base ${
                    ...
                    (watchedIcNumber || watchedPassportNumber) && (icDocumentUrl || passportDocumentUrl)
                      ? 'bg-orange-600 hover:bg-orange-700 shadow-lg'
                      : 'bg-gray-400 cursor-not-allowed'
                  } ${isMobile ? 'font-semibold' : ''}`}
                  disabled={isSubmitting || !(watchedIcNumber || watchedPassportNumber) || !(icDocumentUrl || passportDocumentUrl)}
                  isLoading={isSubmitting}
                >
                  {isMobile ? '📱 Complete Check-in' : 'Complete Check-in'}
                </Button>
              </div>
